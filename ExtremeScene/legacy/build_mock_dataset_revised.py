...
class BatchMockConfig:
    ...
    # ===== 输出目录 =====
    output_dir: str = str(BASE_DIR / "mock_dataset_outputs")
...
def make_random_mock_data(
    ...
    daylight_mask = ((hour >= 6) & (hour <= 18)).astype(float)

    irradiance = _daylight_profile(hour, peak=520.0) * cloud_factor
    irradiance += rng.normal(0, 18.0, n) * daylight_mask
    irradiance = np.clip(irradiance, 0.0, None)
    irradiance[daylight_mask == 0] = 0.0
    ...
    solar_power = 0.46 * irradiance + rng.normal(0, 8.0, n) * daylight_mask
    solar_power = np.clip(solar_power, 0.0, None)
    solar_power[daylight_mask == 0] = 0.0
    ...
    night_mask = ~((df["time"].dt.hour >= 6) & (df["time"].dt.hour <= 18))
    df.loc[night_mask, "irradiance"] = 0.0
    df.loc[night_mask, "solar_power"] = 0.0
...
def main() -> None:
    batch_cfg = BatchMockConfig(
        ...
        merge_overlap=False,
        output_dir=str(BASE_DIR / "mock_dataset_outputs"),
    )
