...
class TrainConfig:
    ...
    out_dir: str = str(BASE_DIR / "mock_full_diffusion_evt_risk_outputs_v2")
    seed: int = 42
    ...
    lambda_risk: float = 0.12
    lambda_cum: float = 1.0
    lambda_ramp: float = 0.7
    lambda_dur: float = 0.6
    duration_temp: float = 12.0
    use_severity_aux: bool = True

    # 工程增强：提升图像效果
    lambda_recon: float = 0.03
    lambda_resource: float = 0.08
    daylight_start_hour: int = 6
    daylight_end_hour: int = 18
    solar_threshold: float = 5.0
...
class SequenceConditionDataset(Dataset):
    def __init__(self, data_dir: Path, seq_len: int = 24, use_severity_aux: bool = True,
                 daylight_start_hour: int = 6, daylight_end_hour: int = 18) -> None:
        ...
        self.meta_df = pd.read_csv(meta_path) if meta_path.exists() else None
        self.use_severity_aux = use_severity_aux
        self.daylight_start_hour = daylight_start_hour
        self.daylight_end_hour = daylight_end_hour
        ...
        self.day_masks, self.hour_sin, self.hour_cos = self._build_time_masks(seq_len)
        self.cond_features, self.cond_meta = self._build_condition_matrix(self.cond_df)
        self.target_metrics, self.risk_stats = self._build_target_metrics(self.cond_df)
    ...
    def _build_time_masks(self, seq_len: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        n = len(self.cond_df)
        day_masks = np.ones((n, seq_len), dtype=np.float32)
        hour_sin = np.zeros((n, 1), dtype=np.float32)
        hour_cos = np.zeros((n, 1), dtype=np.float32)

        if self.meta_df is None or "window_start_time" not in self.meta_df.columns:
            return day_masks, hour_sin, hour_cos

        start_ts = pd.to_datetime(self.meta_df["window_start_time"])
        start_hours = start_ts.dt.hour.to_numpy(dtype=np.int64)
        hour_sin[:, 0] = np.sin(2 * np.pi * start_hours / 24.0)
        hour_cos[:, 0] = np.cos(2 * np.pi * start_hours / 24.0)

        for i, h0 in enumerate(start_hours):
            hours = (h0 + np.arange(seq_len)) % 24
            mask = ((hours >= self.daylight_start_hour) & (hours <= self.daylight_end_hour)).astype(np.float32)
            day_masks[i] = mask
        return day_masks, hour_sin, hour_cos
    ...
    def _build_condition_matrix(self, df: pd.DataFrame) -> Tuple[np.ndarray, dict]:
        ...
        tail_score = (-np.log(p_ext)).astype(np.float32)
        tail_score = ((tail_score - tail_score.mean()) / (tail_score.std() + 1e-6))[:, None]
        extremeness = (1.0 - p_ext)[:, None]
        ...
        cond_parts = [event_onehot, low_wind, low_irr, p_ext_col, extremeness, tail_score]
        feature_layout = {
        ...
        cond_parts.extend([duration_z[:, None], month_sin, month_cos, self.hour_sin, self.hour_cos])
        feature_layout["duration_hours_zscore"] = [next_idx]
        feature_layout["month_sin"] = [next_idx + 1]
        feature_layout["month_cos"] = [next_idx + 2]
        feature_layout["start_hour_sin"] = [next_idx + 3]
        feature_layout["start_hour_cos"] = [next_idx + 4]
    ...
    @staticmethod
    def _build_target_metrics(df: pd.DataFrame) -> Tuple[np.ndarray, dict]:
        ...
        risk_stats = {
            "log_cum_mean": float(np.log1p(metrics[:, 0]).mean()),
            "log_cum_std": float(np.log1p(metrics[:, 0]).std() + 1e-6),
            "ramp_mean": float(metrics[:, 1].mean()),
            "ramp_std": float(metrics[:, 1].std() + 1e-6),
            "dur_mean": float(metrics[:, 2].mean()),
            "dur_std": float(metrics[:, 2].std() + 1e-6),
        }
        return metrics, risk_stats
    ...
    def __getitem__(self, idx: int):
        ...
        day_mask = torch.from_numpy(self.day_masks[idx])
        return x, cond, metrics, day_mask
...
def compute_resource_consistency_loss(x_proj: torch.Tensor, x_true: torch.Tensor, day_mask: torch.Tensor, cond: torch.Tensor, layout: dict) -> torch.Tensor:
    low_wind_idx = layout["low_wind_flag"][0]
    low_irr_idx = layout["low_irradiance_flag"][0]
    low_wind = cond[:, low_wind_idx]
    low_irr = cond[:, low_irr_idx]
...
def plot_loss_curve(history: dict[str, list[float]], out_path: Path) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Conditional DDPM Training Loss (EVT + Tail + Risk + Physics)")
    plt.legend()
    plt.tight_layout()
...
def format_demo_text(meta: pd.Series, event_name: str, best_val: float, best_mae: Optional[float] = None) -> tuple[str, str]:
    line1 = f"EVT-guided DDPM v2 | sample_id={meta.get('sample_id', 'N/A')} | event={event_name}"
    line2 = (
...
def evaluate_epoch(model: nn.Module, loader: DataLoader, scheduler: DiffusionScheduler, device: torch.device,
                   x_mean_t: torch.Tensor, x_std_t: torch.Tensor, cfg: TrainConfig, risk_stats: dict, feature_layout: dict):
    model.eval()
    vstats = {"total": 0.0, "eps": 0.0, "risk": 0.0, "recon": 0.0, "resource": 0.0, "n": 0}
    with torch.no_grad():
        for x, cond, metrics, day_mask in loader:
            x = x.to(device)
            cond = cond.to(device)
            ...
            cum_pred, ramp_pred, dur_pred = compute_risk_metrics_from_seq(x_proj, cfg.duration_temp)
            target_cum = metrics[:, 0]
            target_ramp = metrics[:, 1]
            target_dur = metrics[:, 2]
            risk_cum = F.l1_loss((torch.log1p(cum_pred) - risk_stats["log_cum_mean"]) / risk_stats["log_cum_std"],
                                 (torch.log1p(target_cum) - risk_stats["log_cum_mean"]) / risk_stats["log_cum_std"])
            risk_ramp = F.l1_loss((ramp_pred - risk_stats["ramp_mean"]) / risk_stats["ramp_std"],
                                  (target_ramp - risk_stats["ramp_mean"]) / risk_stats["ramp_std"])
            risk_dur = F.l1_loss((dur_pred - risk_stats["dur_mean"]) / risk_stats["dur_std"],
                                 (target_dur - risk_stats["dur_mean"]) / risk_stats["dur_std"])
            ...
            resource_loss = compute_resource_consistency_loss(x_proj, x_true, day_mask, cond, feature_layout)
            total_loss = eps_loss + cfg.lambda_risk * risk_loss + cfg.lambda_recon * recon_loss + cfg.lambda_resource * resource_loss

            bs = x.size(0)
            vstats["total"] += total_loss.item() * bs
            vstats["eps"] += eps_loss.item() * bs
            vstats["risk"] += risk_loss.item() * bs
            vstats["recon"] += recon_loss.item() * bs
            vstats["resource"] += resource_loss.item() * bs
            vstats["n"] += bs
    return {k: v / max(1, vstats["n"]) for k, v in vstats.items() if k != "n"}
...
def main(cfg: TrainConfig) -> None:
    ...
    risk_stats = {k: torch.tensor(v, dtype=torch.float32, device=device) for k, v in dataset.risk_stats.items()}

    history = {"train_total": [], "train_eps": [], "train_risk": [], "train_recon": [], "train_resource": [],
               "val_total": [], "val_eps": [], "val_risk": [], "val_recon": [], "val_resource": []}
    best_val = float("inf")

    for epoch in range(1, cfg.epochs + 1):
        model.train()
        stats = {"total": 0.0, "eps": 0.0, "risk": 0.0, "recon": 0.0, "resource": 0.0, "n": 0}

        for x, cond, metrics, day_mask in train_loader:
            x = x.to(device)
            cond = cond.to(device)
            metrics = metrics.to(device)
            day_mask = day_mask.to(device)
            ...
            recon_loss = F.smooth_l1_loss(x_proj, x_true)
            resource_loss = compute_resource_consistency_loss(x_proj, x_true, day_mask, cond, dataset.cond_meta["feature_layout"])
            total_loss = eps_loss + cfg.lambda_risk * risk_loss + cfg.lambda_recon * recon_loss + cfg.lambda_resource * resource_loss
            ...
            stats["recon"] += recon_loss.item() * bs
            stats["resource"] += resource_loss.item() * bs
            stats["n"] += bs

        for key in ["total", "eps", "risk", "recon", "resource"]:
            history[f"train_{key}"].append(stats[key] / max(1, stats["n"]))

        ema.copy_to(ema_model)
        vstats = evaluate_epoch(ema_model, val_loader, scheduler, device, x_mean_t, x_std_t, cfg, risk_stats, dataset.cond_meta["feature_layout"])
        for key, value in vstats.items():
            history[f"val_{key}"].append(value)

        if vstats["total"] < best_val:
            best_val = vstats["total"]
            torch.save(
                {
                    "model_state": ema_model.state_dict(),
                    "config": asdict(cfg),
                    "cond_meta": dataset.cond_meta,
                    "risk_stats": dataset.risk_stats,
                    "x_mean": dataset.X_mean,
                    "x_std": dataset.X_std,
                },
                out_dir / "best_evt_risk_ddpm_v2.pt",
            )

        if epoch == 1 or epoch % 10 == 0 or epoch == cfg.epochs:
            print(
                f"Epoch {epoch:03d}/{cfg.epochs} | train_total={history['train_total'][-1]:.4f} | "
                f"train_eps={history['train_eps'][-1]:.4f} | train_risk={history['train_risk'][-1]:.4f} | "
    ...
    meta_df = pd.read_csv(data_dir / "meta.csv") if (data_dir / "meta.csv").exists() else None
    ...
    demo_day_mask = torch.from_numpy(dataset.day_masks[demo_idx: demo_idx + 1]).to(device)
    ...
    # 推理后施加物理投影，改善夜间光伏问题
    multi_gen_t = torch.from_numpy(multi_gen).to(device)
    multi_gen_proj = apply_physical_projection(multi_gen_t, demo_day_mask.repeat(candidate_num, 1)).cpu().numpy()
    maes = np.mean(np.abs(multi_gen_proj - real_seq[None, :, :]), axis=(1, 2))
    best_idx = int(np.argmin(maes))
    gen_seq = multi_gen_proj[best_idx]
    gen_mean_seq = multi_gen_proj.mean(axis=0)

    meta_row = cond_df.loc[demo_idx].copy()
    if meta_df is not None and len(meta_df) == len(cond_df):
        for col in meta_df.columns:
            if col not in meta_row.index:
                meta_row[col] = meta_df.loc[demo_idx, col]
    ...
    np.save(out_dir / "generated_samples.npy", multi_gen_proj.astype(np.float32))
    summary = {
        "device": str(device),
    ...
    print("- best_evt_risk_ddpm_v2.pt")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="加入 EVT 连续概率引导 + 尾部敏感训练 + 联合失衡约束，并补充物理投影的条件扩散模型")
    parser.add_argument("--data-dir", type=str, default=str(BASE_DIR / "mock_dataset_outputs"))
    parser.add_argument("--out-dir", type=str, default=str(BASE_DIR / "mock_full_diffusion_evt_risk_outputs_v2"))
    parser.add_argument("--epochs", type=int, default=120)
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--lr", type=float, default=8e-4)
    parser.add_argument("--steps", type=int, default=100)
    parser.add_argument("--guidance", type=float, default=1.8)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--lambda-risk", type=float, default=0.12)
    parser.add_argument("--tail-alpha", type=float, default=2.2)
    parser.add_argument("--tail-gamma", type=float, default=1.0)
    parser.add_argument("--lambda-recon", type=float, default=0.03)
    parser.add_argument("--lambda-resource", type=float, default=0.08)
