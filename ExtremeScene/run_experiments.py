...
MAIN_COMPARE_METHODS = ["traditional_gaussian_copula", "plain_diffusion_baseline", "improved_diffusion", "enhanced_gan", "proposed"]
ABLATION_METHODS = ["proposed", "no_evt_strict", "no_evt", "no_risk_loss", "no_month", "flat_condition"]
...
def _run_plain_diffusion_baseline(data_dir: Path, model_dir: Path, cfg: ExperimentConfig) -> Path:
    ...
    _run_external_pipeline(script, [
        "pipeline", "--data-dir", str(data_dir), "--output-dir", str(model_dir),
        "--epochs", str(cfg.plain_epochs), "--batch-size", str(cfg.batch_size),
        "--diffusion-steps", str(max(cfg.diffusion_steps, 64)),
        "--base-channels", str(max(cfg.base_channels, 48)),
        "--seed", str(cfg.seed), "--device", cfg.device,
    ])
...
def _run_enhanced_gan_baseline(data_dir: Path, model_dir: Path, cfg: ExperimentConfig) -> Path:
    ...
    _run_external_pipeline(script, [
        "pipeline", "--data-dir", str(data_dir), "--output-dir", str(model_dir),
        "--epochs", str(cfg.gan_epochs), "--batch-size", str(cfg.batch_size),
        "--n-per-condition", "1", "--device", cfg.device, "--seed", str(cfg.seed),
    ])
    return model_dir / "generation" / "generated_samples.npy"
...
def _run_traditional_copula_baseline(data_dir: Path, model_dir: Path, cfg: ExperimentConfig) -> Path:
    ...
    _run_external_pipeline(script, [
        "pipeline", "--data-dir", str(data_dir), "--output-dir", str(model_dir),
        "--n-per-condition", "1", "--seed", str(cfg.seed), "--seq-len", str(cfg.seq_len),
    ])
    return model_dir / "generation" / "generated_samples.npy"


def _train_and_generate(data_dir: Path, model_dir: Path, method_name: str, cfg: ExperimentConfig) -> Path:
    if method_name in {"proposed", "proposed_pretrain", "proposed_riskshape_aug"}:
        ablation = "full"; s1, s2, s3 = cfg.stage1_epochs, cfg.stage2_epochs, cfg.stage3_epochs
        lambda_tail, lambda_risk = cfg.lambda_tail, cfg.lambda_risk
        ...
        ablation = "flat_condition"; s1, s2, s3 = cfg.stage1_epochs + cfg.stage2_epochs + cfg.stage3_epochs, 0, 0
        lambda_tail, lambda_risk = 0.0, 0.0
    elif method_name == "improved_diffusion":
        ablation = "no_risk_loss"; s1, s2, s3 = cfg.stage1_epochs, cfg.stage2_epochs, cfg.stage3_epochs
        lambda_tail, lambda_risk = cfg.lambda_tail, 0.0
    else:
        raise ValueError(f"Unsupported trainable method: {method_name}")

    train_cfg = TrainConfig(
        data_dir=str(data_dir), out_dir=str(model_dir), ablation=ablation, seq_len=cfg.seq_len,
        batch_size=cfg.batch_size, lr=cfg.learning_rate, weight_decay=cfg.weight_decay,
        diffusion_steps=cfg.diffusion_steps, base_channels=cfg.base_channels,
        guidance_scale=cfg.guidance_scale, cond_dropout=cfg.cond_dropout, ema_decay=cfg.ema_decay,
        stage1_epochs=s1, stage2_epochs=s2, stage3_epochs=s3,
        lambda_tail=lambda_tail, lambda_risk=lambda_risk, lambda_cum=cfg.lambda_cum,
        lambda_ramp=cfg.lambda_ramp, lambda_dur=cfg.lambda_dur, lambda_recon=cfg.lambda_recon,
        lambda_physics=cfg.lambda_physics, lambda_resource=cfg.lambda_resource,
        device=cfg.device, seed=cfg.seed, use_augmented_train=cfg.use_augmented_train,
        use_pretrain=cfg.use_pretrain or method_name == "proposed_pretrain",
        ...
        freeze_after_pretrain=cfg.freeze_after_pretrain,
        pretrain_loss_mode=cfg.pretrain_loss_mode,
        augmented_train_dir=cfg.augmented_train_dir,
        use_risk_shapelet_aug=cfg.use_risk_shapelet_aug or method_name == "proposed_riskshape_aug",
        risk_shapelet_aug_dir=cfg.risk_shapelet_aug_dir,
    )
    train_model(train_cfg)
    condition_guided = method_name in {
        "proposed",
        "no_evt",
        "no_evt_continuous",
        "no_evt_strict",
    ...
    checkpoint_type = cfg.checkpoint_type if condition_guided else "best"
    guidance_scale = cfg.guidance_scale if condition_guided else None
    generate_from_checkpoint(GenerationConfig(
        checkpoint=None, data_dir=str(data_dir), out_dir=str(model_dir), split="test",
        guidance_scale=guidance_scale, checkpoint_type=checkpoint_type,
    ))
...
def run_experiments(cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    for method in cfg.methods:
        ...
        try:
            ...
            eval_summary = evaluate_generation(EvalConfig(
                real=str(data_dir / "X_test.npy"), generated=str(generated_path),
                cond=str(data_dir / "cond_test.csv"), meta=str(data_dir / "meta_test.csv"),
                out_dir=str(eval_dir), model_name=method,
...
def parse_args() -> ExperimentConfig:
    ...
    if args.methods is None:
        if args.preset == "main":
            ...
            methods = list(dict.fromkeys(MAIN_COMPARE_METHODS + ABLATION_METHODS))
        else:
            methods = ["proposed"]
    else:
        methods = list(args.methods)
    return ExperimentConfig(
        data_dir=args.data_dir, out_dir=args.out_dir, methods=methods, seq_len=args.seq_len,
        stage1_epochs=args.stage1_epochs, stage2_epochs=args.stage2_epochs, stage3_epochs=args.stage3_epochs,
        batch_size=args.batch_size, diffusion_steps=args.diffusion_steps, base_channels=args.base_channels,
        plain_epochs=args.plain_epochs, gan_epochs=args.gan_epochs, device=args.device,
        guidance_scale=args.guidance_scale, lambda_tail=args.lambda_tail, lambda_risk=args.lambda_risk,
        lambda_cum=args.lambda_cum, lambda_ramp=args.lambda_ramp, lambda_dur=args.lambda_dur,
        lambda_recon=args.lambda_recon, lambda_physics=args.lambda_physics, lambda_resource=args.lambda_resource,
        cond_dropout=args.cond_dropout, ema_decay=args.ema_decay, learning_rate=args.learning_rate,
        weight_decay=args.weight_decay, checkpoint_type=args.checkpoint_type, use_augmented_train=args.use_augmented_train,
        use_pretrain=args.use_pretrain, pretrain_data_dir=args.pretrain_data_dir, stage0_epochs=args.stage0_epochs,
        pretrain_lr=args.pretrain_lr, pretrain_checkpoint=args.pretrain_checkpoint,
        freeze_after_pretrain=args.freeze_after_pretrain, pretrain_loss_mode=args.pretrain_loss_mode,
        augmented_train_dir=args.augmented_train_dir,
        use_risk_shapelet_aug=args.use_risk_shapelet_aug, risk_shapelet_aug_dir=args.risk_shapelet_aug_dir,
        seed=args.seed,
    )
