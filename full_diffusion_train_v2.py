...
class TrainConfig:
    ...
    out_dir: str = "mock_full_diffusion_outputs"
    seed: int = 42
...
def format_demo_text(meta: pd.Series, event_name: str, best_val: float, best_mae: Optional[float] = None) -> tuple[str, str]:
    line1 = f"Conditional DDPM | sample_id={meta.get('sample_id', 'N/A')} | event={event_name}"
    line2 = (
        f"severity={int(meta.get('severity_level', 0))}, low_wind={int(meta.get('low_wind_flag', 0))}, "
        f"low_irr={int(meta.get('low_irradiance_flag', 0))}, duration={int(meta.get('duration_hours', 0))}h, "
        f"month={int(meta.get('month', 0))}, val_loss={best_val:.4f}"
    )
...
def main(cfg: TrainConfig) -> None:
    ...
    sort_cols = ["severity_level", "low_wind_flag", "low_irradiance_flag", "duration_hours"]
    for c in sort_cols:
        if c not in val_meta.columns:
            val_meta[c] = 0
    val_meta = val_meta.sort_values(sort_cols, ascending=[False, False, False, False])
    ...
    # 同条件生成多次，选与真实样本 MAE 最小的一条用于展示，同时保存均值曲线
    ...
    event_code = int(meta_row["event_type_code"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", type=str, default="mock_full_diffusion_outputs")
