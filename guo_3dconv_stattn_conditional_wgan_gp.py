...
def _enforce_solar_night_zero(hourly: np.ndarray, cfg: GanPipelineConfig) -> np.ndarray:
    ...
    if out.ndim == 3 and out.shape[-1] >= 3:
        hh = np.arange(out.shape[1], dtype=np.int32)
        night = (hh < h0) | (hh >= h1)
        out[:, night, 2] = 0.0
...
def _generate_month_pools(gen: nn.Module, scaler: ChannelMinMaxScaler, cfg: GanPipelineConfig) -> Dict[int, np.ndarray]:
    ...
    with torch.no_grad():
        for m in range(1, 13):
            ...
            pools[m] = _enforce_solar_night_zero(x, cfg)
    return pools
...
def _safe_corrcoef(x: np.ndarray) -> np.ndarray:
    ...
    sd = np.sqrt(np.clip(np.diag(cov), 1e-8, None))
    corr = cov / np.maximum(sd[:, None] * sd[None, :], 1e-12)
    corr = np.clip(corr, -0.999, 0.999)
    np.fill_diagonal(corr, 1.0)
    return corr


def _corr_align_block(x: np.ndarray, h: np.ndarray, strength: float = 0.35) -> np.ndarray:
    if x.shape[0] < 8 or h.shape[0] < 24:
        return x
    x_mu = x.mean(axis=0, keepdims=True)
    x_sd = np.maximum(x.std(axis=0, keepdims=True), 1e-6)
    h_mu = h.mean(axis=0, keepdims=True)
    h_sd = np.maximum(h.std(axis=0, keepdims=True), 1e-6)

    zx = (x - x_mu) / x_sd
    zh = (h - h_mu) / h_sd
    if not (np.all(np.isfinite(zx)) and np.all(np.isfinite(zh))):
        return x

    rx = _safe_corrcoef(zx)
    rh = _safe_corrcoef(zh)
    t = _sym_psd_power(rx, -0.5) @ _sym_psd_power(rh, 0.5)
    z_new = zx @ t
    z_mix = (1.0 - strength) * zx + strength * z_new
    x_new = x_mu + z_mix * x_sd

    # Keep block mean/std close to original, so correlation repair doesn't over-distort std curves.
    x_new = x_new + (x_mu - x_new.mean(axis=0, keepdims=True))
    sd_new = np.maximum(x_new.std(axis=0, keepdims=True), 1e-6)
    gamma = np.clip(x_sd / sd_new, 0.85, 1.15)
    x_new = x_new.mean(axis=0, keepdims=True) + gamma * (x_new - x_new.mean(axis=0, keepdims=True))
    return x_new


def _monthly_hourly_corr_align(
    hourly: np.ndarray,
    hist_y: np.ndarray,
    month_doy: np.ndarray,
    strength: float = 0.35,
    ...
    hist_day = hist_y.reshape(hist_y.shape[0], 365, 24, 3)
    if by_month:
        for m in range(1, 13):
            ...
            for h in range(24):
                xb = out[d_idx, h, :]
                hb = hist_day[:, d_idx, h, :].reshape(-1, 3)
                out[d_idx, h, :] = _corr_align_block(xb, hb, strength=strength)
    else:
        for h in range(24):
            xb = out[:, h, :]
            hb = hist_day[:, :, h, :].reshape(-1, 3)
            out[:, h, :] = _corr_align_block(xb, hb, strength=strength)
        ...
        for m in range(1, 13):
            ...
            for c in range(3):
                for h in range(24):
                    ...
                    gamma = float(np.clip(gamma, 0.20, 1.80))
...
def _postprocess_metrics(seq: np.ndarray, hist_y: np.ndarray, cfg: GanPipelineConfig, refs: dict | None = None) -> dict:
    seq = _enforce_solar_night_zero(seq, cfg)
    if refs is None:
...
def _apply_cost_shift(seq: np.ndarray, delta: float, mode: str) -> np.ndarray:
    ...
    if mode == "load":
        ...
        out[:, 0] += eta * delta
        ren = out[:, 1] + out[:, 2]
        share_w = np.zeros_like(ren, dtype=np.float64)
        np.divide(out[:, 1], ren, out=share_w, where=ren > 1e-9)
        share_w = np.where(ren > 1e-9, share_w, 0.5)
...
def _refine_sequence(
    ...
    if not cfg.use_postprocess_refine:
        return _enforce_solar_night_zero(hourly, cfg), {"chosen": "raw"}
    ...
    if cfg.refine_random_trials > 0:
        ...
        for i in range(cfg.refine_random_trials):
            ...
            corr_mode = str(rng.choice(["none", "net", "joint", "hourcorr_m", "hourcorr_g"]))
            if corr_mode == "net":
                corr_s = float(rng.uniform(0.10, 2.20))
            ...
            elif corr_mode == "hourcorr_m":
                corr_s = float(rng.uniform(0.08, 0.70))
                seq = _monthly_hourly_corr_align(seq, hist_y, month_doy, strength=corr_s, by_month=True)
            elif corr_mode == "hourcorr_g":
                corr_s = float(rng.uniform(0.08, 0.70))
        ...
        for key in seed_keys:
            ...
            for s in (0.45, 0.65, 0.85, 1.00):
                for by_month in (True, False):
                    seq = _std_shrink_adjust(base, hist_y, month_doy, strength=float(s), by_month=by_month, only_over=True)
                    seq = _monthly_hourly_corr_align(seq, hist_y, month_doy, strength=0.28, by_month=by_month)
                    seq = _monthly_joint_correction(seq, hist_y, month_doy, strength=0.28)
                    seq = _monthly_quantile_map(seq, hist_y, month_doy, strength=0.06)
                    tag = "m" if by_month else "g"
                    candidates[f"{key}_stdfix_s{s:.2f}_{tag}"] = seq

        # Deterministic correlation-repair candidates for lowering corr/std while keeping other metrics stable.
        corr_seed_keys = [
            "raw",
            "reb_0.90_q_0.50_net",
            "reb_0.90_q_0.50_net_proj_a0.45_joint2",
        ...
        for key in corr_seed_keys:
            base = candidates[str(key)]
            for cs in (0.20, 0.35, 0.50):
                for by_month in (True, False):
                    seq = _monthly_hourly_corr_align(base, hist_y, month_doy, strength=float(cs), by_month=by_month)
                    seq = _std_shrink_adjust(seq, hist_y, month_doy, strength=0.25, by_month=True, only_over=False)
                    seq = _monthly_joint_correction(seq, hist_y, month_doy, strength=0.24)
                    seq = _monthly_quantile_map(seq, hist_y, month_doy, strength=0.06)
                    tag = "m" if by_month else "g"
    ...
    if km_hourly is not None:
        ...
        if not df.empty:
            best_std = str(df.nsmallest(1, "ratio_std_mae_mean")["candidate"].iloc[0])
            best_corr = str(df.nsmallest(1, "ratio_corr_l1")["candidate"].iloc[0])
            best_acf = str(df.nsmallest(1, "ratio_acf_mean")["candidate"].iloc[0])
            best_bal = str(df.nsmallest(1, "max_ratio")["candidate"].iloc[0])
            blend_pairs = [(best_bal, best_corr), (best_bal, best_std), (best_bal, best_acf), (best_std, best_corr)]
            ...
            for a, b in blend_pairs:
                ...
                for lam in (0.35, 0.50, 0.65):
                    seq = lam * xa + (1.0 - lam) * xb
                    seq = _monthly_hourly_corr_align(seq, hist_y, month_doy, strength=0.22, by_month=True)
                    seq = _std_shrink_adjust(seq, hist_y, month_doy, strength=0.18, by_month=True, only_over=False)
                    seq = _monthly_joint_correction(seq, hist_y, month_doy, strength=0.25)
                    seq = _monthly_quantile_map(seq, hist_y, month_doy, strength=0.10)
                    candidates[f"blend_{pair_idx:02d}_l{lam:.2f}"] = seq
                pair_idx += 1
            df = _collect_candidate_metrics(candidates, hist_y, cfg, refs)
            for c in metric_cols:
                df[f"ratio_{c}"] = df[c] / max(float(km_m[c]), 1e-12)
        ...
        if cfg.use_cost_match_candidates and not df.empty:
            ...
            seed_keys.update(df.nsmallest(pick_n, "std_mae_mean")["candidate"].astype(str).tolist())
            seed_keys.update(df.nsmallest(pick_n, "acf_mean")["candidate"].astype(str).tolist())
            seed_keys.update(df.nsmallest(pick_n, "weighted_ratio")["candidate"].astype(str).tolist())
            for key in seed_keys:
                seq0 = candidates[key]
    ...
    return _enforce_solar_night_zero(candidates[best_key], cfg), {"chosen": best_key, "chosen_mode": chosen_mode, "candidates": sorted_rows.to_dict(orient="records")}
